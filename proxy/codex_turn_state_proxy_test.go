		...
		t.Run(fmt.Sprint(resin), func(t *testing.T) {
			...
			for _, ws := range []bool{false, true} {
				for _, ctx := range []context.Context{acquire, verify} {
					e := ResolveCodexRequestEgress(ctx, account, CodexBaseURL+"/responses", "http://normal:8080", ws)
					if e.Kind != CodexEgressProxy || e.DialProxyURL != account.CodexTurnStateProxyURL || e.URL != CodexBaseURL+"/responses" {
