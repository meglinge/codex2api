...
func TestCodexTurnStateCacheSettingsRoundTrip(t *testing.T) {
	...
	recorder = httptest.NewRecorder()
	request := httptest.NewRequest(http.MethodPut, "/api/admin/settings/codex-turn-state-cache", strings.NewReader(`{"ipv6_proxy_url":" socks5://[::1]:1080 ","models":[" gpt-5.6-sol ","gpt-5.6-sol"],"ttl_minutes":43}`))
	...
	if cfg.IPv6ProxyURL != "socks5://[::1]:1080" || len(cfg.Models) != 1 || cfg.Models[0] != "gpt-5.6-sol" || cfg.TTLMinutes != 43 {
		t.Fatalf("cfg = %+v", cfg)
	}
	...
	if cfg.TTLMinutes != 50 || cfg.IPv6ProxyURL != "socks5://[::1]:1080" {
